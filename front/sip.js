                ...
                $("#registerlist a")
                  ...
                  .click(function () {
                    ...
                    switch (selectedApproach) {
                      case "secret":
                        bootbox.alert(
                          "Using this approach you'll provide a plain secret to REGISTER"
                        );
                        break;
                      case "ha1secret":
                        bootbox.alert(
                          "Using this approach might not work with Asterisk because the generated HA1 secret could have the wrong realm"
                        );
                        break;
                      case "guest":
                        bootbox.alert(
                          "Using this approach you'll try to REGISTER as a guest, that is without providing any secret"
                        );
                        break;
                      default:
                        break;
              ...
              onmessage: function (msg, jsep) {
                ...
                if (result && result["event"]) {
                  ...
                  if (event === "registered") {
                    ...
                    if (code == 200) {
                      toastr.success(
                        `${callid} Delivery Status: ${code} ${reason}`
                      );
                    } else {
                      toastr.error(
                        `${callid} Delivery Status: ${code} ${reason}`
              ...
              onremotetrack: function (track, mid, on) {
                ...
                if (
                  ...
                  for (let i = 0; i < 12; i++) {
                    if (i < 10)
                      $("#dtmf").append(
                        '<button class="btn btn-info dtmf">' + i + "</button>"
                      );
                    else if (i == 10)
                      $("#dtmf").append(
                        '<button class="btn btn-info dtmf">#</button>'
                      );
                    else if (i == 11)
                      $("#dtmf").append(
                        '<button class="btn btn-info dtmf">*</button>'
...
function registerUsername() {
  if (!selectedApproach) {
    bootbox.alert(
      "Please select a registration approach from the dropdown menu"
    );
    return;
  }
  // Try a registration
  ...
  if (
    sipserver !== "" &&
    sipserver.indexOf("sip:") != 0 &&
    sipserver.indexOf("sips:") != 0
  ) {
    bootbox.alert(
      "Please insert a valid SIP server (e.g., sip:192.168.0.1:5060)"
    );
    $("#server").removeAttr("disabled");
    $("#username").removeAttr("disabled");
    $("#authuser").removeAttr("disabled");
  ...
  if (selectedApproach === "guest") {
    ...
    if (
      !username === "" ||
      username.indexOf("sip:") != 0 ||
      username.indexOf("@") < 0
    ) {
      bootbox.alert(
        "Please insert a valid SIP address (e.g., sip:goofy@example.com): this doesn't need to exist for guests, but is required"
      );
      $("#server").removeAttr("disabled");
      $("#username").removeAttr("disabled");
      $("#authuser").removeAttr("disabled");
    ...
    if (sipserver === "") {
      bootbox.confirm(
        "You didn't specify a SIP Registrar to use: this will cause the plugin to try and conduct a standard (<a href='https://tools.ietf.org/html/rfc3263' target='_blank'>RFC3263</a>) lookup. If this is not what you want or you don't know what this means, hit Cancel and provide a SIP Registrar instead'",
        function (result) {
          if (result) {
  ...
  if (sipserver === "") {
    bootbox.confirm(
      "You didn't specify a SIP Registrar: this will cause the plugin to try and conduct a standard (<a href='https://tools.ietf.org/html/rfc3263' target='_blank'>RFC3263</a>) lookup. If this is not what you want or you don't know what this means, hit Cancel and provide a SIP Registrar instead'",
      function (result) {
        if (result) {
          sipcall.send({ message: register });
        } else {
...
function doCall(ev) {
  ...
  else helperId = parseInt(helperId);
...
function addHelper(helperCreated) {
  helperCreated =
    typeof helperCreated == "function" ? helperCreated : Janus.noop;
  helpersCount++;
  ...
  $(".footer").before(
    '<div class="container" id="sipcall' +
      helperId +
      '">' +
      '	<div class="row">' +
      '		<div class="col-md-6 container">' +
      '			<span class="badge bg-info">Helper #' +
      helperId +
      '				<i class="fa-solid fa-rectangle-xmark" id="rmhelper' +
      helperId +
      '" style="cursor: pointer;" title="Remove this helper"></i>' +
      "			</span>" +
      "		</div>" +
      '		<div class="col-md-6 container" id="phone' +
      helperId +
      '">' +
      '			<div class="input-group mt-1 mb-1">' +
      '				<span class="input-group-text"><i class="fa-solid fa-phone"></i></span>' +
      '				<input disabled class="form-control" type="text" placeholder="SIP URI to call (e.g., sip:1000@example.com)" autocomplete="off" id="peer' +
      helperId +
      '" onkeypress="return checkEnter(this, event, ' +
      helperId +
      ');"></input>' +
      "			</div>" +
      '			<button disabled class="btn btn-success mb-1" autocomplete="off" id="call' +
      helperId +
      '">Call</button> <input autocomplete="off" id="dovideo' +
      helperId +
      '" type="checkbox">Use Video</input>' +
      "		</div>" +
      "	</div>" +
      '	<div id="videos' +
      helperId +
      '" class="row mt-2 mb-2 hide">' +
      '		<div class="col-md-6">' +
      '			<div class="card">' +
      ...
      "				</div>" +
      '				<div class="card-body" id="videoright' +
      helperId +
      '"></div>' +
      "			</div>" +
      "		</div>" +
  ...
  janus.attach({
    ...
    success: function (pluginHandle) {
      ...
      Janus.log(
        "[Helper #" +
          helperId +
          "] Plugin attached! (" +
          helpers[helperId].sipcall.getPlugin() +
          ", id=" +
          helpers[helperId].sipcall.getId() +
          ")"
      );
    ...
    error: function (error) {
      Janus.error(
        "[Helper #" + helperId + "]   -- Error attaching plugin...",
        error
      );
      bootbox.alert("  -- Error attaching plugin... " + error);
      ...
      if (result && result["event"]) {
        ...
        if (event === "registered") {
          ...
          $("#call" + helperId)
            ...
            .unbind("click")
            .click(doCall);
          if (helperCreated) helperCreated(helperId);
        } else if (event === "calling") {
          Janus.log(
            "[Helper #" + helperId + "] Waiting for the peer to answer..."
          );
          // TODO Any ringtone?
          $("#call" + helperId)
            .removeAttr("disabled")
            ...
            buttons: {
              success: {
                ...
                callback: function () {
                  ...
                  // an offerless call, we'll need to create an offer ourselves
                  let sipcallAction = offerlessInvite
                    ? helpers[helperId].sipcall.createOffer
          ...
          // Call can start already: handle the remote answer
          if (jsep) {
            helpers[helperId].sipcall.handleRemoteJsep({
              jsep: jsep,
              error: function () {
          ...
          if (code == 200) {
            toastr.success(
              `${callid}/${helperId} Delivery Status: ${code} ${reason}`
            );
          } else {
            toastr.error(
              `${callid}/${helperId} Delivery Status: ${code} ${reason}`
    ...
    onlocaltrack: function (track, on) {
      ...
      if (!on) {
        ...
        if (track.kind === "video") {
          ...
          if (helpers[helperId].localVideos === 0) {
            // No video, at least for now: show a placeholder
            if (
              $("#videoleft" + helperId + " .no-video-container").length === 0
            ) {
      ...
      if (track.kind === "audio") {
        ...
        Janus.attachMediaStream(
          $("#myvideo" + helperId + "t" + trackId).get(0),
          stream
        );
      }
      if (
        helpers[helperId].sipcall.webrtcStuff.pc.iceConnectionState !==
          "completed" &&
        helpers[helperId].sipcall.webrtcStuff.pc.iceConnectionState !==
          "connected"
      ) {
    ...
    onremotetrack: function (track, mid, on) {
      ...
      if (!on) {
        ...
        if (track.kind === "video") {
          ...
          if (remoteVideos === 0) {
            // No video, at least for now: show a placeholder
            if (
              $("#videoright" + helperId + " .no-video-container").length === 0
            ) {
              $("#videoright").append(
                '<div class="no-video-container">' +
                  '<i class="fa-solid fa-video fa-xl no-video-icon"></i>' +
      ...
      // If we're here, a new track was added
      if (
        $("#videoright" + helperId + " audio").length === 0 &&
        $("#videoright" + helperId + " video").length === 0
      ) {
        $("#videos" + helperId).removeClass("hide");
        $("#videoright" + helperId)
          .parent()
          .find("span")
          .html(
            'Send DTMF: <span id="dtmf' +
              helperId +
              '" class="btn-group btn-group-xs"></span>' +
              '<span id="ctrls" class="top-right btn-group btn-group-xs">' +
              '<button id="msg' +
              helperId +
              '" title="Send message" class="btn btn-info"><i class="fa-solid fa-envelope"></i></button>' +
              '<button id="info' +
              helperId +
              '" title="Send INFO" class="btn btn-info"><i class="fa-solid fa-info"></i></button>' +
              '<button id="transfer' +
              helperId +
              '" title="Transfer call" class="btn btn-info"><i class="fa-solid fa-share"></i></button>' +
              "</span>"
          );
        for (let i = 0; i < 12; i++) {
          if (i < 10)
            $("#dtmf" + helperId).append(
              '<button class="btn btn-info dtmf">' + i + "</button>"
            );
          else if (i == 10)
            $("#dtmf" + helperId).append(
      ...
      if (track.kind === "audio") {
        ...
        Janus.log(
          "[Helper #" + helperId + "] Created remote audio stream:",
          stream
        );
        ...
        Janus.attachMediaStream(
          $("#peervideo" + helperId + "m" + mid).get(0),
          stream
        );
        ...
        helpers[helperId].remoteTracks[mid] = stream;
        Janus.log(
          "[Helper #" + helperId + "] Created remote video stream:",
          stream
        );
        $("#videoright" + helperId).append(
          '<video class="rounded centered" id="peervideo' +
            helperId +
            "m" +
            mid +
            '" width="100%" height="100%" autoplay playsinline/>'
    ...
    oncleanup: function () {
      Janus.log(
        "[Helper #" + helperId + "]  ::: Got a cleanup notification :::"
      );
      $("#videoleft" + helperId)
        .empty()
        .parent()
